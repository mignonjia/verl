...
LOG_DIR="${HOME}/hapo/logs"
mkdir -p "$LOG_DIR"
LOG_FILE="${LOG_DIR}/qwen2.5-hapo-$(date +%Y%m%d-%H%M%S).log"
...
python3 -m verl.trainer.main_ppo \
    ...
    +trainer.hapo_debug=True \
    trainer.critic_warmup=0 \
    trainer.rollout_data_dir="$ROLLOUT_DATA_DIR" \
    trainer.logger='["console","wandb"]' \
    trainer.project_name='verl_example' \
    trainer.experiment_name='Qwen2.5-7B-hapo' \
    trainer.n_gpus_per_node=4 \
    trainer.nnodes=1 \
    trainer.save_freq=-1 \
    trainer.test_freq=10 \
    trainer.total_epochs=10 $@ 2>&1 | tee -a "$LOG_FILE"
